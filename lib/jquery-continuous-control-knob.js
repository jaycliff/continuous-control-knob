        ...
        (function () {
            ...
            */
            function documentEventHandler(event) {
                var nowX, nowY, now_radians, changedTouches, touch_object, k, len, cancel = true;
                switch (event.type) {
                case 'touchmove':
            ...
            cckWrapMetaControlHandler = (function () {
                ...
                return function cckWrapMetaControlHandler(event) {
                    var event_type = event.type, changedTouches, touch_object;
            ...
            continuous_control_knob_object.enable = function enable() {
                if (disabled === true) {
                    disabled = false;
                    $continuous_control_knob_object.on('focus blur touchstart touchmove touchend mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckEventHandler);
                    $cck_wrap
                        .removeClass('disabled')
                        .on('focus blur touchstart touchmove touchend mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckWrapMetaControlHandler)
                        .attr('tabindex', tab_index)
            ...
            continuous_control_knob_object.disable = function disable() {
                if (disabled === false) {
                    disabled = true;
                    $continuous_control_knob_object.off('focus blur touchstart touchmove touchend mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckEventHandler);
                    if (active) {
                        $document.trigger('mouseup'); // Manually trigger the 'mouseup' event handler
                    }
                    $cck_wrap
                        .addClass('disabled')
                        .off('focus blur touchstart touchmove touchend mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckWrapMetaControlHandler)
                        .removeAttr('tabindex')
