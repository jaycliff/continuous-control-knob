...
(function createContinuousControlKnobSetup(window, $, undef) {
    "use strict";
    ...
    $.createContinuousControlKnob = function createContinuousControlKnob(options) {
        ...
        function displayBands(show) {
            if (arguments.length > 0) {
                if (!!show) {
                    $cck_bands_wrap[0].style.display = '';
                } else {
                    $cck_bands_wrap.hide();
                }
        ...
        function useVerticalSwipe(show) {
            if (arguments.length > 0) {
                if (!!show) {
                    use_vertical_swipe = true;
                } else {
                    use_vertical_swipe = false;
                }
                //$cck_bands_wrap.toggle(!!show);
            ...
            return use_vertical_swipe;
        }
        function displayLabels(show) {
            if (arguments.length > 0) {
                if (!!show) {
                    $cck_label_wrap[0].style.display = '';
                } else {
                    $cck_label_wrap.hide();
                }
        ...
        function initializeParts() {
            ...
            if (is_options_valid) {
                ...
                if (hasOwnProperty.call(options, 'useVerticalSwipe')) {
                    use_vertical_swipe = !!options.useVerticalSwipe;
                }
        ...
        $continuous_control_knob_object = $({
            ...
            val: function val(user_val) {
                var max_sub, min_sub, step_sub, rate;
                if (arguments.length > 0) {
                    max_sub = properties.max;
                    min_sub = properties.min;
            ...
            useVerticalSwipe: useVerticalSwipe,
        ...
        Object.defineProperty(continuous_control_knob_object, 'value', {
            ...
            set: function (val) {
                var step_sub = properties.step, max_sub = properties.max, min_sub = properties.min, rate;
        ...
        */
        ...
        (function () {
            var knob_center = {
                ...
                cck_do_not_trigger_map = {},
                cck_wrap_do_not_trigger_map = {},
                prevX,
                prevY,
                touch_identifier = -1;
            ...
            // Updates the knob UI
            updateControls = function refresh(animate) {
                ...
                $cck_value.text(properties.value.toFixed(decimalDigitsLength(properties.step)));
                // highlight bands
            ...
            */
            ...
            function calculateAngleAndRate(mode, direction) {
                var rate, min_sub, step_sub, event;
                switch (mode) {
                case 'indirect':
                    switch (direction) {
                    case 'up':
                        ...
                        if (rate > 1) {
                            ...
                            event = calculateAngleAndRate._event;
                            if (event) {
                                event.preventDefault();
                                event.stopPropagation();
                                calculateAngleAndRate._event = undef;
                            }
                        }
                        angle = rate * maxangle;
                        inputEvent(rate);
            ...
            function docWinEventHandler() {
                release();
                $document.off('mousemove mouseup', documentEventHandler);
            ...
            */
            function calculateRotationalMovement(nowX, nowY) {
                var now_radians = getRadians(nowX, nowY);
            ...
            function documentEventHandler(event) {
                ...
                case 'mousemove':
                    ...
                    if (use_vertical_swipe) {
                        ...
                        calculateRotationalMovement(nowX, nowY);
                    }
                    prevX = nowX;
                    prevY = nowY;
                    break;
            ...
            cckWrapMetaControlHandler = (function () {
                var is_default_prevented = false, is_propagation_stopped = false, is_immediate_propagation_stopped = false, cck_anchor = $cck_anchor[0];
                ...
                return function cckWrapMetaControlHandler(event) {
                    var nowX, nowY, event_type = event.type, changedTouches, touch_object, k, len, cancel = true;
                    // trigger's extra parameters won't work with focus and blur events. See https://github.com/jquery/jquery/issues/1741
                    if (!cck_do_not_trigger_map[event_type]) {
                        cck_wrap_do_not_trigger_map[event_type] = true;
                        $continuous_control_knob_object.one(event_type, helper).triggerHandler(event_type);
                        cck_wrap_do_not_trigger_map[event_type] = false;
                    }
                    ...
                    case 'touchstart':
                        ...
                        if (containsTarget(event.target, cck_anchor)) {
                            event.preventDefault();
                            active = true;
                    ...
                    case 'mousedown':
                        ...
                        if (containsTarget(event.target, cck_anchor)) {
                            ...
                            prev_radians = getRadians(prevX, prevY);
                            $cck_wrap.trigger('focus').addClass('active');
                            $document.on('mousemove mouseup', documentEventHandler);
                    ...
                    case 'DOMMouseScroll':
                        ...
                        if (containsTarget(event.target, cck_anchor)) {
                            if (event.originalEvent) {
                                calculateAngleAndRate._event = event;
            ...
            function cckEventHandler(event) {
                ...
                if (!cck_wrap_do_not_trigger_map[event_type]) {
                    //console.log('triggered ' + event_type);
                    cck_do_not_trigger_map[event_type] = true;
                    $cck_wrap.trigger(event_type);
                    cck_do_not_trigger_map[event_type] = false;
                }
            }
            continuous_control_knob_object.enable = function enable() {
                if (disabled === true) {
                    disabled = false;
                    // $continuous_control_knob_object's attached events should also be found on $cck_wrap's cckWrapMetaControlHandler
                    $continuous_control_knob_object.on('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckEventHandler);
                    $cck_wrap
                        .removeClass('disabled')
                        .attr('tabindex', tab_index)
                        .on('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckWrapMetaControlHandler)
            ...
            continuous_control_knob_object.disable = function disable() {
                if (disabled === false) {
                    disabled = true;
                    // $continuous_control_knob_object's attached events should also be found on $cck_wrap's cckWrapMetaControlHandler
                    $continuous_control_knob_object.off('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckEventHandler);
                    if (active) {
                        docWinEventHandler();
                    }
                    $cck_wrap
                        .addClass('disabled')
                        .removeAttr('tabindex')
                        .off('focus blur touchstart touchmove touchend touchcancel mousewheel DOMMouseScroll mousedown mousemove mouseup click keydown keyup keypress', cckWrapMetaControlHandler)
                        .on('mousedown touchstart', enableDisableAid);
                    removeTransitionClass();
                }
                return continuous_control_knob_object;
            };
            continuous_control_knob_object.refresh = updateControls;
